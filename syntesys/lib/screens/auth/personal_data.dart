...
class _PersonalDataState extends LoadingBaseState<PersonalData> {
  ...
  Widget buildFirstNameTextField() => new Container(
    ...
    child: new TextField(
      ...
      decoration: new InputDecoration.collapsed(hintText: "Primeiro Nome"),
  ...
  Widget buildLastNameTextField() => new Container(
    ...
    child: new TextField(
      ...
      decoration: new InputDecoration.collapsed(hintText: "Último Nome"),
      keyboardType: TextInputType.text,
    ),
  );
  ...
  void _save() {
    ...
    if (firstName.length>0 && firstName.length<2){
      AlertDialog d = DialogShower.buildDialog(message: "Primeiro nome é muito pequeno",confirm: "Ok", confirmFn: ()=>Navigator.pop(context));
      showDialog(context: context, child: d);
      return;
    }
    if(lastName.length>0 && lastName.length<2){
      AlertDialog d = DialogShower.buildDialog(message: "Último nome é muito pequeno",confirm: "Ok", confirmFn: ()=>Navigator.pop(context));
      showDialog(context: context, child: d);
      return;
    }
    if(lastName.length == 0 && firstName.length == 0) {
      showDialog(context: context,
          child: DialogShower.buildDialog(message: "Sem conteúdo para salvar",confirm: "Ok", confirmFn: ()=>Navigator.pop(context)));
      return;
    }
    String displayName="";
